...
- create/update the file versions1/latest-pre-release without new-line and setup 2.0.0 version folder
```
mkdir versions1/2.0.0v
echo -n 2.0.0 > versions1/2.0.0v/latest-pre-release
cp versions1/certs.sector* versions1/2.0.0v
cp versions1/public*key*   versions1/2.0.0v
```
- set local.mk to the ota-main program
```
make -j6 rebuild OTAVERSION=2.0.0
mv firmware/otamain.bin versions1/2.0.0v
```
- set local.mk back to ota-boot program
```
make -j6 rebuild OTAVERSION=2.0.0
mv firmware/otaboot.bin versions1/2.0.0v
make -j6 rebuild OTAVERSION=2.0.0 OTABETA=1
cp firmware/otaboot.bin versions1/2.0.0v/otabootbeta.bin
```
- remove the older version files
#
- update Changelog
- commit this as version 2.0.0  
- set up a new github release 2.0.0 as a pre-release using the just commited master...  
...
```
make flash OTAVERSION=2.0.0 OTABETA=1
```
...
```
openssl sha384 versions1/2.0.0v/otamain.bin
xxd versions1/2.0.0v/otamain.bin.sig
```

- upload the file versions1/2.0.0v/latest-pre-release to the 'latest release' assets on github
...
```
cp  versions1/2.0.0v/public-1.key.sig versions1
```
...
```
cp  versions1/2.0.0v/certs.sector.sig versions1
```
