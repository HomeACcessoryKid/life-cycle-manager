...
- create/update the file versions1/latest-pre-release without new-line and setup 0.9.14 version folder
```
echo -n 0.9.14 > versions1/0.9.14v/latest-pre-release
mkdir versions1/0.9.14v
cp versions1/certs.sector* versions1/0.9.14v
cp versions1/public*key*   versions1/0.9.14v
```
- set local.mk to the ota-main program
```
make -j6 rebuild OTAVERSION=0.9.14
mv firmware/otamain.bin versions1/0.9.14v
```
- set local.mk back to ota-boot program
```
make -j6 rebuild OTAVERSION=0.9.14
mv firmware/otaboot.bin versions1/0.9.14v
make -j6 rebuild OTAVERSION=0.9.14 OTABETA=1
cp firmware/otaboot.bin versions1/0.9.14v/otabootbeta.bin
```
- remove the older version files
#
- commit this as version 0.9.14  
- set up a new github release 0.9.14 as a pre-release using the just commited master...  
...
```
make flash OTAVERSION=0.9.14 OTABETA=1
```
...
```
openssl sha384 versions1/0.9.14v/otamain.bin
xxd versions1/0.9.14v/otamain.bin.sig
```

- upload the file versions1/0.9.14v/latest-pre-release to the 'latest release' assets on github
...
```
cp  versions1/0.9.14v/public-1.key.sig versions1
```
...
```
cp  versions1/0.9.14v/certs.sector.sig versions1
```
