...
```
make -j6 rebuild OTAVERSION=0.9.15
mv firmware/otamain.bin versions1/0.9.15v
```
- set local.mk back to ota-boot program
```
make -j6 rebuild OTAVERSION=0.9.15
mv firmware/otaboot.bin versions1/0.9.15v
make -j6 rebuild OTAVERSION=0.9.15 OTABETA=1
cp firmware/otaboot.bin versions1/0.9.15v/otabootbeta.bin
```
- remove the older version files
#
- commit this as version 0.9.15  
- set up a new github release 0.9.15 as a pre-release using the just commited master...  
...
```
make flash OTAVERSION=0.9.15 OTABETA=1
```
...
```
openssl sha384 versions1/0.9.15v/otamain.bin
xxd versions1/0.9.15v/otamain.bin.sig
```

- upload the file versions1/0.9.15v/latest-pre-release to the 'latest release' assets on github
...
```
cp  versions1/0.9.15v/public-1.key.sig versions1
```
...
```
cp  versions1/0.9.15v/certs.sector.sig versions1
```
